Require Import Util PString Lia Rewrite ZArith Strl.
Require I2N.
...
Section Proof.
  ...
  Proof.
    ...
    assert (List.length (LO padded) = List.length (LO bin) + to_nat pad)%nat.
      ...
      subst. I2N.zify. change (to_Z 4095) with (Z.ones 12). clear; rewrite Z.land_ones; lia.
    ...
    I2N.zify. rewrite of_Z_spec, Z.add_mod_idemp_l, Z.mod_small. lia. split. lia.
    apply slltl in Maxlen. rewrite Zlength_correct in Maxlen. enough (pad <? 0x1000 = true). lia.
    subst pad. I2N.zify. change (to_Z 4095) with (Z.ones 12). clear; rewrite Z.land_ones; lia. lia.
  Qed.
End Proof.
